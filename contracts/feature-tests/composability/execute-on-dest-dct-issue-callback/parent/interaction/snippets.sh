...
executeOnDestIssueToken() {
    mxpy --verbose contract call ${ADDRESS} --recall-nonce --pem=${ALICE} --gas-limit=200000000 --value=${DCT_ISSUE_COST} --function="executeOnDestIssueToken" --arguments ${TOKEN_DISPLAY_NAME} ${TOKEN_TICKER} ${INITIAL_SUPPLY} --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
...
getWrappedMoaxTokenIdentifier() {
    getChildContractAddress
    mxpy --verbose contract call ${CHILD_ADDRESS} --recall-nonce --pem=${ALICE} --gas-limit=50000000 --function="getWrappedMoaxTokenIdentifier" --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
